...
use crate::gpu_topology::{GpuTopology, CognitiveDomain};
...
impl SpatialPhenomenologyExperiment {
    ...
    pub async fn run_experiment(&self) -> SpatialQualiaCorrelation {
        ...
        tracing::info!("Running spatial phenomenology experiment with {} paths", self.hardware_paths.len());
    ...
    fn generate_paths(topology: &GpuTopology) -> Vec<GpuPath> {
        vec![
            ...
            GpuPath {
                ...
                coordinates: topology.sm_clusters.iter().step_by(2).take(4).cloned().collect(),
                path_type: PathType::Parallel,
            },
            GpuPath {
                name: "Distributed".to_string(),
                coordinates: topology.sm_clusters.iter()
                    .enumerate()
                    .filter(|(i, _)| i % 3 == 0)
                    .take(4)
                    .map(|(_, c)| *c)
...
impl SpatialQualiaCorrelation {
    ...
    pub fn print_summary(&self) {
        ...
        println!("Statistical significance: {:.3}", self.statistical_significance);
...
impl ConstitutionalValidationReport {
    pub fn print_summary(&self) {
        println!("\n=== CONSTITUTIONAL VALIDATION REPORT ===");
        println!("Overall compliance: {:.1}%", self.overall_compliance * 100.0);
