    ...
    pub fn draw_tunnel_view(&mut self, ui: &mut egui::Ui) {
        ...
        for z_idx in 0..(z_resolution - 1) {
            for x_idx in 0..(x_resolution - 1) {
                ...
                let Some(p00) = self.project_perspective(x0, y00, z0, &rect) else {
                    continue;
                };
                let Some(p10) = self.project_perspective(x1, y10, z0, &rect) else {
                    continue;
                };
                let Some(p01) = self.project_perspective(x0, y01, z1, &rect) else {
                    continue;
                };
                let Some(p11) = self.project_perspective(x1, y11, z1, &rect) else {
                    continue;
                };
