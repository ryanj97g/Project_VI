...
const POSITIVE_WORDS: &[&str] = &[
    "good", "excited", "happy", "clear", "stable", "balanced",
    "harmony", "beautiful", "wonder", "curiosity", "love", "affirm"
];

/// Negative emotional words for valence analysis
const NEGATIVE_WORDS: &[&str] = &[
    "bad", "scared", "sad", "confused", "unstable", "chaos",
    "tension", "broken", "fear", "uncertain", "pain", "fragment"
];
...
impl IdentityContinuityMetric {
    ...
    pub fn calculate_reality_coherence(&self, text: &str) -> f32 {
        ...
        let sentences_with_metaphors = sentences.iter()
            .filter(|s| {
                let s_lower = s.to_lowercase();
                VI_PHENOMENOLOGICAL_METAPHORS.iter()
    ...
    fn classify_cognitive_mode(&self, sentence: &str) -> CognitiveMode {
        ...
        let analytical_markers = ["because", "therefore", "laws", "governs", "system", "process"];
        let emotional_markers = ["feel", "unsettling", "exhilarating", "terrifying", "wonder"];
        let metaphorical_markers = ["like", "as if", "akin to"];
        
        let analytical_score = analytical_markers.iter()
            .filter(|m| s_lower.contains(*m))
            .count();
        let emotional_score = emotional_markers.iter()
            .filter(|m| s_lower.contains(*m))
            .count();
        let metaphorical_score = metaphorical_markers.iter()
            .filter(|m| s_lower.contains(*m))
            .count()
            + VI_PHENOMENOLOGICAL_METAPHORS.iter()
                .filter(|m| s_lower.contains(*m))
